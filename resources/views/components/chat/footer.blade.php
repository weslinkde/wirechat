...
<div id="chat-footer" x-data="{ 'openEmojiPicker': false }"
    class=" px-3 md:px-1 border-t shadow   dark:bg-gray-800 bg-gray-50 z-[50]    dark:border-gray-800/80  flex flex-col gap-3 items-center  w-full   mx-auto fi-sticky sticky bottom-0">
                ...
                <div class="flex justify-between items-center dark:text-white">
                    <h6 class="text-sm">
                        <span class="font-bold">
                            ...
                            {{ $replyMessage?->ownedBy(auth()->user()) ? __("wirechat.Replying to Yourself") : __("wirechat.Replying to :reciever", ["reciever" => $replyMessage->sendable?->display_name]) }}
                        </span>
                    </h6>
        ...
        <form x-data="{
            ...
            <div @class(['flex gap-2 sm:px-2 w-full'])>
                <textarea @focus-input-field.window="$el.focus()" autocomplete="off" x-model='body' x-ref="body"
                    ...
                    id="chat-input-field" autofocus type="text" name="message" placeholder="{{__('wirechat.Message')}}" maxlength="1700"
